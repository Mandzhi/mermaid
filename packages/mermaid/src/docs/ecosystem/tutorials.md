...
Here's an example of Python integration with mermaid-js which uses the mermaid.ink service, that displays the graph in a Jupyter notebook / Python IDLE and allows to save it as .png image with the provided resolution (in this example, it's equal to 1200: `dpi=1200`).
...
def mm(graph):
    ...
    plt.imshow(img)
    plt.axis('off') # needs to hide axis
